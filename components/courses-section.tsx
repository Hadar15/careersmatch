...
export function CoursesSection({ searchTerm, noContainer }: CoursesSectionProps) {
  ...
  if (noContainer) {
    return (
      <div className="container mx-auto px-0">
        ...
        >
            {courses
              ...
              .filter(course => {
                // Filter by searchTerm jika ada
                if (!searchTerm || searchTerm.trim() === "") return true;
                const q = searchTerm.toLowerCase();
                return (
                  course.name.toLowerCase().includes(q) ||
                  course.description.toLowerCase().includes(q) ||
                  (course.primaryLanguages && course.primaryLanguages.some(l => l.toLowerCase().includes(q)))
                );
              })
              .map((course, idx) => (
      ...
      {loading && <div className="text-center py-6 text-sky-600 font-semibold">Memuat...</div>}
      {!hasMore && !loading && (
        <div className="text-center py-6 text-gray-400">Tidak ada lagi kursus.</div>
      )}
    </div>
  );
  }
  // Default: dengan container border dan background putih (landing page)
  return (
    <div className="container mx-auto px-0">
